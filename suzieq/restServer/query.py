...
def rest_main(*args) -> None:
    ...
    api_key_mode = cfg.get("rest", {}).get("api-key-mode", "cfg")
    if api_key_mode == "cfg":
        single_key = cfg.get("rest", {}).get("API_KEY")
        multi_key = cfg.get("rest", {}).get("API_KEYS")
        if not single_key and not multi_key:
            print("missing API_KEY or API_KEYS in config file")
            sys.exit(1)
    elif api_key_mode == "env":
        keys = os.environ.get("SUZIEQ_API_KEYS")
        if not keys:
            print("missing SUZIEQ_API_KEYS in environment")
            sys.exit(1)
    else:
        print("Invalid api-key-mode in config file")
        sys.exit(1)
...
def missing_verb(command):
    return_error(
        404, f"{command} command missing a verb. for example " f"/api/v2/{command}/show"
    )
